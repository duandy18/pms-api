# app/pms/suppliers/repos/supplier_repo.py
from __future__ import annotations

from sqlalchemy import exists, or_
from sqlalchemy.orm import Session, selectinload

from app.pms.suppliers.models.supplier import Supplier
from app.pms.suppliers.models.supplier_contact import SupplierContact


def list_suppliers(
    db: Session,
    *,
    active: bool | None = None,
    q: str | None = None,
) -> list[Supplier]:
    query = db.query(Supplier).options(selectinload(Supplier.contacts))

    if active is not None:
        query = query.filter(Supplier.active.is_(bool(active)))

    if q and q.strip():
        pattern = f"%{q.strip()}%"
        base_match = or_(Supplier.name.ilike(pattern), Supplier.code.ilike(pattern))
        contact_match = exists().where(
            (SupplierContact.supplier_id == Supplier.id)
            & (
                SupplierContact.name.ilike(pattern)
                | SupplierContact.phone.ilike(pattern)
                | SupplierContact.email.ilike(pattern)
                | SupplierContact.wechat.ilike(pattern)
            )
        )
        query = query.filter(or_(base_match, contact_match))

    return query.order_by(Supplier.id.asc()).all()


def list_suppliers_basic(
    db: Session,
    *,
    active: bool | None = True,
    q: str | None = None,
) -> list[Supplier]:
    query = db.query(Supplier)

    if active is not None:
        query = query.filter(Supplier.active.is_(bool(active)))

    if q and q.strip():
        pattern = f"%{q.strip()}%"
        query = query.filter(or_(Supplier.name.ilike(pattern), Supplier.code.ilike(pattern)))

    return query.order_by(Supplier.id.asc()).all()


def get_supplier_with_contacts(db: Session, supplier_id: int) -> Supplier | None:
    return (
        db.query(Supplier)
        .options(selectinload(Supplier.contacts))
        .filter(Supplier.id == int(supplier_id))
        .one_or_none()
    )


def create_supplier(
    db: Session,
    *,
    name: str,
    code: str,
    website: str | None,
    active: bool,
) -> Supplier:
    supplier = Supplier(name=name, code=code, website=website, active=bool(active))
    db.add(supplier)
    db.commit()
    db.refresh(supplier)
    return supplier


def save_supplier(db: Session, supplier: Supplier) -> Supplier:
    db.commit()
    db.refresh(supplier)
    return supplier
