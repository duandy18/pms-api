...
router = APIRouter(tags=["pms-suppliers"])
...
@router.get("/pms/suppliers", response_model=list[SupplierOut])
def list_suppliers(
    ...
    return [_to_supplier_out(supplier) for supplier in suppliers]


@router.post("/pms/suppliers", response_model=SupplierOut, status_code=status.HTTP_201_CREATED)
def create_supplier(
...
@router.patch("/pms/suppliers/{supplier_id}", response_model=SupplierOut)
def update_supplier(
